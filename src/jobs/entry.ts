...
import { loadCachedScrips, verifyAndGetLotSize, resolveT0AndT1, ScripItem } from '../helpers/scripMaster.js';
...
export async function runEntrySequence(): Promise<void> {
  ...
  try {
    ...
    T0 = resolved.T0;
    T1 = resolved.T1;
    console.log(`[ENTRY] Resolved Expiries -> T0 (Current): ${getISTDateString(T0)}, T1 (Next): ${getISTDateString(T1)}`);
  } catch (err: any) {
    await sendAlert(`🚨 Aborting entry: Failed to resolve T0 and T1 expiries. Error: ${err.message}`);
    return;
  }
  ...
  const findOptionContract = (strike: number, optionType: 'CE' | 'PE', expiryStr: string): ScripItem | undefined => {
    return scrips.find(
      (item) =>
        item.name === 'NIFTY' &&
        item.exch_seg === 'NFO' &&
  ...
  if (!longCEContract || !longPEContract) {
    await sendAlert(`🚨 Aborting entry: Long T1 option contracts not found in scrip master for strikes CE ${longCEStrike}, PE ${longPEStrike} on ${T1ScripStr}`);
    return;
  }
  ...
  try {
    ...
    longCEPremium = await confirmOrderFill(longCEOrderId, longCEContract.symbol, longCEContract.token);
    longPEPremium = await confirmOrderFill(longPEOrderId, longPEContract.symbol, longPEContract.token);
    
    console.log(`[ENTRY] Long fills confirmed -> CE: ₹${longCEPremium}, PE: ₹${longPEPremium}`);
  } catch (err: any) {
    await sendAlert(`🚨 CRITICAL: Long strangle entry failed or could not be confirmed: ${err.message}. Aborting entire entry sequence.`);
  ...
  try {
    ...
    console.log(`[ENTRY] Selected short CE: ${shortCEContract.symbol} (strike ${Math.round(parseFloat(shortCEContract.strike)/100)})`);
    console.log(`[ENTRY] Selected short PE: ${shortPEContract.symbol} (strike ${Math.round(parseFloat(shortPEContract.strike)/100)})`);
  } catch (err: any) {
    await sendAlert(`🚨 Phase C failed: Strike selection for short hedges failed: ${err.message}. Keeping long positions open. Position state set to PARTIAL_ENTRY.`);
    return;
  }
  ...
  let shortCEOrderId = '';
  let shortPEOrderId = '';
  let shortCEPremium = 0;
  let shortPEPremium = 0;
  ...
  try {
    ...
    shortCEPremium = await confirmOrderFill(shortCEOrderId, shortCEContract.symbol, shortCEContract.token);
    shortPEPremium = await confirmOrderFill(shortPEOrderId, shortPEContract.symbol, shortPEContract.token);

    console.log(`[ENTRY] Short fills confirmed -> CE: ₹${shortCEPremium}, PE: ₹${shortPEPremium}`);
  } catch (err: any) {
    // Rule 1.2: Partial entry policy
    await sendAlert(`🚨 CRITICAL PARTIAL ENTRY: Long legs filled, but short hedges failed or could not be confirmed: ${err.message}. Position state: PARTIAL_ENTRY.`);
